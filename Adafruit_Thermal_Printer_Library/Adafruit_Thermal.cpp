...
void Adafruit_Thermal::begin(uint16_t version) {
  ...
  //TODO: set heat config doesn't work - need to do manually?
  //setHeatConfig();
  ...
  //dotPrintTime = 15000; // See comments near top of file for
  //dotFeedTime = 2100;   // an explanation of these values.
  dotPrintTime = 7000; // See comments near top of file for
  dotFeedTime = 1000;   // an explanation of these values.
...
void Adafruit_Thermal::printBarcode(const char *text, uint8_t type) {
  ...
  if (firmware >= 264)
    type += 65;
  writeBytes(ASCII_GS, 'H', 0);    // Print no label for barcode
  ...
  timeoutSet((barcodeHeight + 40) * (dotPrintTime*0.5));
  prevByte = '\n';
}
...
void Adafruit_Thermal::setHeatConfig(uint8_t dots, uint8_t time, uint8_t interval) {
  writeBytes(ASCII_ESC, '7');       // Esc 7 (print settings)
  writeBytes(dots, time, interval); // Heating dots, heat time, heat interval

  //writeBytes(ASCII_ESC, '7',dots,time,interval);
}
